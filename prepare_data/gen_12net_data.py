...
flags.DEFINE_string('log_file', 'gen_12net_data', 'log filename')
...
def main(_):
    ...
    anno_file = cfg['pnet_anno_file']
    im_dir = cfg['pnet_im_dir']
    pos_save_dir = cfg['pnet_pos_save_dir']
    part_save_dir = cfg['pnet_part_save_dir']
    neg_save_dir = cfg['pnet_neg_save_dir']
    save_dir = cfg['pnet_save_dir']
    ...
    for annotation in annotations:
        ...
        while neg_num < 50:
            ...
            if np.max(Iou) < 0.3:
                ...
                f2.write(f"{save_dir}/negative/{n_idx}.jpg 0\n")
        ...
        for box in boxes:
            ...
            for i in range(20):
                ...
                if iou  >= 0.65:
                    ...
                    f1.write(f"{save_dir}/positive/{p_idx}.jpg 1 " + 
                             '%.2f %.2f %.2f %.2f\n' % (offset_x1, offset_y1, 
                                                        offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    p_idx += 1
                    ...
                    f3.write(f"{save_dir}/part/{d_idx}.jpg -1 " + 
                             '%.2f %.2f %.2f %.2f\n' % (offset_x1, offset_y1, 
                                                        offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    d_idx += 1
            box_idx += 1
            if idx % 100 == 0:
                alog.info("%s images done, pos: %s part: %s neg: %s" % (idx, p_idx, d_idx, n_idx))
    f1.close()
    f2.close()
    f3.close()
