...
anno_file = r'..\data\celebA\Anno\list_bbox_celeba.txt'
landmark_file = r'..\data\celebA\Anno\list_landmarks_celeba.txt'
im_dir = r'..\data\celebA\Img\img_celeba'
...
# Divide data into batches, and id_train is the batch number
id_train = 1
max_data_size = int(num / 20)
start_idx = (id_train - 1) * max_data_size
end_idx = min(id_train * max_data_size, num)
annotations = annotations[start_idx:end_idx]
num = len(annotations)

print ("id_train: {:d}, start: {:d}, end: {:d}".format(id_train, 
       start_idx, end_idx))
...
fid = open(r"..\data\mtcnn_training\{0:d}net\roi{1:d}.imdb".format(
            image_size, id_train),'wb')
pickle.dump(roi_list, fid)
fid.close()
roi_list = []

fid = open(r"..\data\mtcnn_training\{0:d}net\pts{1:d}.imdb".format(
            image_size, id_train),'wb')
pickle.dump(pts_list, fid)
fid.close()
pts_list = []

fid = open(r"..\data\mtcnn_training\{0:d}net\cls{1:d}.imdb".format(
            image_size, id_train),'wb')
