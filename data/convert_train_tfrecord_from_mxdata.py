...
def main(_):
    ...
    with tf.io.TFRecordWriter(FLAGS.output_path) as writer:
        ...
        for i in tqdm.tqdm(img_idx_list):
            ...
            img_filename = os.path.join(img_folder, f'{img_id}.jpg').replace("\\", "/")
        ...
        for img_id, img_label, img_filename in tqdm.tqdm(samples):
            ...
            img_train_path = os.path.join(FLAGS.training_dataset_path, 
                                          img_filename).replace("\\", "/") 
            
            tf_example = make_example(source_id=img_label,
                                      img_path=str.encode(img_train_path))
